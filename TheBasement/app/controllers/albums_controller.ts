...
import { urlAlbumValidator, updateAlbumValidator } from '#validators/album'
...
export default class AlbumsController {
  ...
  async albumId({ view, params }: HttpContext) {
    ...
    const songs = await db
      ...
      .where('albums.album_id', params.albumId)
      .select('songs.*')
  ...
  async storeAlbum({ request, response }: HttpContext) {
    ...
    album.price = Number.parseFloat((50 + Math.random() * 50).toFixed(2))
    ...
    album.duration = Number.parseFloat((duration / 60000).toFixed(2))
    ...
    const res_songs: any = await fetch(
      `https://api.spotify.com/v1/albums/${payload}/tracks`,
      options
    )
