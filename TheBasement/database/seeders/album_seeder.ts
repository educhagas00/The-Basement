  ...
  async run() {
    ...
    for (const albumId of albumIds) {
      try {
        ...
        album.price = Number.parseFloat((50 + Math.random() * 50).toFixed(2)) // numero aleatorio de 50 a 100 com 2 casas decimais
        let duration = 0
        albumData.tracks.items.forEach((track: any) => {
          duration += track.duration_ms
        })
        album.duration = Number.parseFloat((duration / 60000).toFixed(2)) // duration em minutos
